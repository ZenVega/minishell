...
/*   Updated: 2025/05/23 12:28:42 by uschmidt         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static const char	*get_error_msg(t_err_code err)
{
	...
	errors[10] = (t_err){.code = ERR_IS_NO_FOLDER, .msg = ERR_IS_NO_FOLDER_MSG};
	errors[11] = (t_err){.code = ERR_PERM, .msg = ERR_PERM_MSG};
	errors[12] = (t_err){.code = ERR_IDENT, .msg = ERR_IDENT_MSG};
	i = -1;
	while (++i < ERR_COUNT)
...
int	exit_with_error(t_cmd_info cmd)
{
	...
	else if (cmd.err_info.code == ERR_SIGINT)
		return (-1);
	else
